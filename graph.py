...
class UndirectedGraph:
    def __init__(self):
        ...
        # Takes in a pair of points as the key and the value is the edge class
        self.edges = {}
        self.edge_group = pygame.sprite.Group()
        self.textbox_group = pygame.sprite.LayeredUpdates()
    ...
    def removePoint(self, location):
        ...
        for other_point in self.adjacency_list[closest_point]:
            # Delete the edges
            self.edges.pop((closest_point, other_point)).getTextBox().kill()
            self.edges.pop((other_point, closest_point)).kill()
    ...
    def getTextBoxesClicked(self, location):
        return self.textbox_group.get_sprites_at(location)
    ...
    def addEdge(self):
        ...
        edge = Edge(self.edge_group, self.selected_point1, self.selected_point2, self.textbox_group)
        # Create the edge weights
        self.edges[(self.selected_point1, self.selected_point2)] = edge
        self.edges[(self.selected_point2, self.selected_point1)] = edge

    def deleteEdge(self):
        if self.selected_point2 is None or self.selected_point1 not in self.adjacency_list[self.selected_point2]:
            return

        # Both pops return the same edge class, so we use it to kill all the classes
        self.edges.pop((self.selected_point1, self.selected_point2)).getTextBox().kill()
        self.edges.pop((self.selected_point2, self.selected_point1)).kill()
    ...
    def updateEdges(self):
        self.edge_group.update()

    def updateTextBoxes(self):
        self.textbox_group.update()
