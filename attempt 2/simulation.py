...
class Simulation:
    def __init__(self):
        ...
        for i in range(NUM_VEHICLES):
            self.vehicles.append(Vehicle(random.randint(1, N_VERTICES), random.randint(1, N_VERTICES), self.graph, self))
    ...
    def generateGraph(self):
        ...
        for i in range(M_EDGES):
            self.graph.addEdge(random.randint(1, N_VERTICES), random.randint(1, N_VERTICES), random.random() * MAX_ROAD_LENGTH + MIN_ROAD_LENGTH)
                    ...
                    if i == iterations - 1:
                        ...
                        for j in range(1, len(p[0])):
                            ...
                            commonScore += self.graph.edgeWeight(u, v) * (1 + (MAX_TRAFFIC_MULTIPLIER - 1)
                                        * math.pow(self.globalTrafficDensity[min(u - 1, v - 1)][max(u - 1, v - 1)], TRAFFIC_MULTIPLIER_EXPONENT)
