...
class Vehicle:
    ...
    def iterate(self):
        ...
        for i in range(1, len(bestPath[0])):
            ...
            self.pheromones[min(u - 1, v - 1)][max(u - 1, v - 1)] += constants.PHEROMONE_DEPOSIT_CONSTANT * bestScore/bestPath[1]
        ...
        if constants.DEPOSIT_PHEROMONES_ON_GLOBAL_BEST and self.simulation.bestSolutionScore != float('inf'):
            for i in range(1, len(self.simulation.bestSolution[self.id][0])):
                v = self.simulation.bestSolution[self.id][0][i - 1]
                u = self.simulation.bestSolution[self.id][0][i]

                # Only decays on pheromone updates for paths -> updates all past times missed
                if constants.EFFICIENT_DECAY_RATES:
                    self.pheromones[min(u - 1, v - 1)][max(u - 1, v - 1)] *= math.pow(constants.PHEROMONE_DECAY_RATE,
                                                                                      self.timesDecayed -
                                                                                      self.lastDecayed[
                                                                                          min(u - 1, v - 1)][
                                                                                          max(u - 1, v - 1)])
                ...
                # Updates pheromones
                self.pheromones[min(u - 1, v - 1)][max(u - 1, v - 1)] = max(self.pheromones[min(u - 1, v - 1)][max(u - 1, v - 1)], constants.MAX_PHEROMONE_FOR_GLOBAL_BEST_PATH)
